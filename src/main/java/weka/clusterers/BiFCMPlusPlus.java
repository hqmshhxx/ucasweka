...
public class BiFCMPlusPlus {
	...
	public void multipleNodeDeletion() {
		...
		array=new Instances[4];
		newArray=new Instances[4];
		for(int i=0;i<4;i++){
			array[i]=new Instances(plain,0);
			newArray[i]=new Instances(plain,0);
			if(i<3){
		...
		array[3]=new Instances(plain);
		instances=null;
		statisticClass();
		showBiclusterCenters();
		try {
			distributeFCM();
		} catch (Exception e) {
...
//		saveIns();
	...
	public void singleNodeDeletion() {
		...
		if (maxH[0] > maxH[1]) {
			...
			}else if(plainCols<=instances.numAttributes()+3){
				array[1].add(plain.instance(maxRowIndex));
			}else{
				array[2].add(plain.instance(maxRowIndex));
			}
...
*/
	...
	public void showBiclusterCenters(){
		System.out.println("biclustering centroids:");
		for(int i=0; i<array.length; i++){
			for(int j=0; j<array[0].numAttributes()-1; j++){
				System.out.print(array[i].meanOrMode(j)+" ");
			}
			System.out.println();
	...
	public void toCentroids(){
		...
		centroidIns[0].addAll(newArray[0]);
		for(int i =1;i<3; i++){
			centroidIns[1].addAll(newArray[i]);
		}
		centroidIns[2].addAll(newArray[3]);
...
/*
	...
	*/	
		...
		for(int i=0; i<centroidIns.length; i++){
			Instance in=new DenseInstance(centroidIns[0].numAttributes()-1);
			for(int j=0; j<centroidIns[0].numAttributes()-1; j++){
...
/*		
	...
	*/	
	...
	public void distributeFCM() throws Exception{
		
		for(int i=1; i<array.length;i++){
			if(!array[i].isEmpty()){
				FuzzyCPlusPlus fcpp =new FuzzyCPlusPlus();
				fcpp.setPlainData(array[i]);
				fcpp.buildClusterer(array[i]);
				newArray[i].addAll(fcpp.getNewData());
			}
		}
	}
	public void lastFCM(){
		FuzzyC fc=new FuzzyC(matrix);
		try {
			fc.buildClusterer(getData());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
//		fc.evalution();
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		BiFCMPlusPlus bi = new BiFCMPlusPlus();
