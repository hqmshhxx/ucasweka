...
public class FuzzyCMeans extends RandomizableClusterer
...
/*		
		for(int i=0;i<instances.numInstances(); i++){
			for(int j=0; j<m_NumClusters; j++){
				System.out.print(memberShip.get(i, j)+" ");
			}
			System.out.println();
		}
*/
...
*/
	...
	private double[] calculateMeansOrMediansOrModes(int centroidIndex, Instances members, boolean updateClusterInfo) {
...
//				m_ClusterCentroids.set(k, in);
	...
	private void updateMemberShip(Instances instances) {
		...
		for (int j = 0; j < m_NumClusters; j++) {
			...
			for (int i = 0; i < instances.numInstances(); i++) {
				bottom = 0;
				top = m_DistanceFunction.distance(instances.instance(i), m_ClusterCentroids.instance(j));
				sum = 0;
				for (int k = 0; k < m_NumClusters; k++) {
					bottom = m_DistanceFunction.distance(instances.instance(i), m_ClusterCentroids.instance(k));
